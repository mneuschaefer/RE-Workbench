...
2. Include Custom File Explorer sorting as recommended. Offer File Color as an optional visual preference; setup and all Workbench functions work without it. Follow their configuration in `7 Tools/README.md`, including the generated source-order file, folder colours and readability check. If File Color is chosen, read `7 Tools/obsidian-defaults.json` and apply its palette, folder assignments, background colouring and inheritance through the installed plugin settings; highlight only `1 Working files` and its descendants with the subtle background tint. Leave templates, notes, archives, import logs and tools uncoloured. Remove earlier Workbench demo colour assignments for those folders when upgrading; preserve unrelated personal settings. File Color assignments reference palette IDs, not colour objects. Merge these defaults without removing unrelated user settings. Offer Commander only if the user wants toolbar shortcuts. Preserve existing themes and unrelated plugin settings. Install third-party plugins through Obsidian rather than copying another person's `.obsidian` directory.
