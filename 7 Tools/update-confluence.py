...
import os,json,re,hashlib,base64,urllib.request,urllib.parse,urllib.error,sys,shutil,tempfile,time,email.utils
...
ORIGIN=''
API_MODE='cloud'  # resolved to 'cloud' or 'server' (Data Center / self-hosted)
CONTENT_FORMAT=6
...
def load_config():
 ...
 configured_mode=c.get('apiMode','auto')
 if configured_mode not in ('auto','cloud','server'):raise RuntimeError('apiMode must be auto, cloud or server')
 if site.scheme!='https' or site.username or site.password or not host or site.query or site.fragment:raise RuntimeError('Use an HTTPS Confluence site URL without credentials, query or fragment')
 api_mode=('cloud' if re.fullmatch(r'[a-z0-9-]+\.atlassian\.net',host) else 'server') if configured_mode=='auto' else configured_mode
 if api_mode=='cloud' and not re.fullmatch(r'[a-z0-9-]+\.atlassian\.net',host):raise RuntimeError('Use an HTTPS Atlassian Cloud site URL')
 if api_mode=='cloud' and site.path not in ('','/'):raise RuntimeError('Cloud site URL must contain only the site address')
 base_path='' if site.path in ('','/') else site.path.rstrip('/')
 if '//' in base_path or any(urllib.parse.unquote(part) in ('.','..') for part in base_path.split('/')):raise RuntimeError('Invalid Confluence context path')
 ...
 return {'siteUrl':site.scheme+'://'+site.netloc+base_path,'spaceKey':str(c['spaceKey']),'scope':{'mode':mode,'pageIds':sorted(set(map(str,ids))) if mode!='space' else []},'apiMode':api_mode}
def confined(rel):
 p=(ROOT/rel).resolve()
...
def retry_delay(error,attempt):
 ...
 if value:
  if value.isdigit():return min(60,max(0,int(value)))
  try:
   target=email.utils.parsedate_to_datetime(value).timestamp()
   return min(60,max(0,target-time.time()))
  except (TypeError,ValueError,OverflowError):pass
 return min(8,2**attempt)
def req(path):
 global API_MODE
 if API_MODE=='server':
  if not re.match(r'^/(content|space)(?:/[A-Za-z0-9_-]+)?(?:/(?:child/page))?(?:[/?]|$)',path) or '..' in path:raise RuntimeError('Unexpected API path for server mode')
 else:  # cloud mode
  if not re.match(r'^/(pages|folders|spaces)(?:/[0-9]+)?(?:[/?]|$)',path) or '..' in path:raise RuntimeError('Unexpected API path')

 if API_MODE=='server':
  auth_header='Bearer '+os.environ['REWB_TOKEN']
  base_url=ORIGIN+'/rest/api'
  api_path=path
 else:  # cloud mode
  auth=base64.b64encode((os.environ['REWB_EMAIL']+':'+os.environ['REWB_TOKEN']).encode()).decode()
  auth_header='Basic '+auth
  base_url=ORIGIN+'/wiki/api/v2'
  api_path=path

 q=urllib.request.Request(base_url+api_path,headers={'Authorization':auth_header,'Accept':'application/json'},method='GET')
 for attempt in range(MAX_ATTEMPTS):
  try:
   ...
   code=error.code
   if code in RETRYABLE_HTTP and attempt+1<MAX_ATTEMPTS:
    delay=retry_delay(error,attempt);error.close();time.sleep(delay);continue
   error.close()
   if code==401:raise RuntimeError('Confluence authentication failed. Check the API token or personal access token') from None
   if code==403:raise RuntimeError('Confluence denied read access. Check the account and space permissions') from None
   if code==429:raise RuntimeError('Confluence rate limit remained active after retries') from None
   if code in RETRYABLE_HTTP:raise RuntimeError('Confluence remained temporarily unavailable after retries') from None
   raise RuntimeError('Confluence request failed with HTTP '+str(code)) from None
  except (urllib.error.URLError,TimeoutError) as error:
   if attempt+1<MAX_ATTEMPTS:
    time.sleep(min(8,2**attempt));continue
   reason=getattr(error,'reason',error)
   label='timeout' if isinstance(reason,TimeoutError) else 'connection error'
   raise RuntimeError('Confluence request failed after retries: '+label) from None
def listing(path):
 global API_MODE
 seen=set();result=[]
 while path:
  if path in seen:raise RuntimeError('Repeated pagination')
  seen.add(path);d=req(path)
  if not isinstance(d,dict):raise RuntimeError('Unexpected Confluence listing response')
  values=d.get('results',d.get('values'))
  if not isinstance(values,list):raise RuntimeError('Unexpected Confluence listing response')
  result.extend(values);path=d.get('_links',{}).get('next')
  if path:
   if path.startswith(ORIGIN):path=path[len(ORIGIN):]
   if API_MODE=='server':
     parsed=urllib.parse.urlsplit(path)
     if parsed.scheme or parsed.netloc:
      if parsed.scheme+'://'+parsed.netloc!=urllib.parse.urlsplit(ORIGIN).scheme+'://'+urllib.parse.urlsplit(ORIGIN).netloc:raise RuntimeError('Unexpected pagination URL for server mode')
      path=parsed.path+('?' + parsed.query if parsed.query else '')
     context=urllib.parse.urlsplit(ORIGIN).path.rstrip('/')
     if context and path.startswith(context+'/rest/api/'):path=path[len(context):]
     if not path.startswith('/rest/api/'):raise RuntimeError('Unexpected pagination URL for server mode')
 ...
 return result
def collect(c):
 global API_MODE, ORIGIN
 if API_MODE=='server':
  space=req('/space/'+urllib.parse.quote(c['spaceKey']))
  if str(space.get('key','')).casefold()!=c['spaceKey'].casefold():raise RuntimeError('Confluence space not found or ambiguous')
  space_id=str(space['id']);mode=c['scope']['mode'];nodes={};pages={};skipped=[];positions={}
  if mode=='space':
   for item in listing('/content?spaceKey='+urllib.parse.quote(c['spaceKey'])+'&type=page&status=current&limit=100&expand=space,version,ancestors'):
    pages[str(item['id'])]=item
  else:
   for cid in c['scope']['pageIds']:
    pages[cid]=req('/content/'+cid+'?expand=space,version,ancestors')
   if mode=='trees':
    queue=[i for i in pages];seen=set()
    while queue:
     cid=queue.pop(0)
     if cid in seen:continue
     seen.add(cid)
     children=listing('/content/'+cid+'/child/page?limit=100&expand=space,version,ancestors')
     for position,child in enumerate(children):
      child_id=str(child['id']);remote_position=child.get('position');positions[child_id]=remote_position if isinstance(remote_position,int) and remote_position>=0 else position;queue.append(child_id)
      if child_id not in pages:pages[child_id]=child
  if mode=='space':
   parent_ids=sorted({str(p['ancestors'][-1]['id']) for p in pages.values() if p.get('ancestors')})
   for parent_id in parent_ids:
    for position,child in enumerate(listing('/content/'+parent_id+'/child/page?limit=100&expand=space,version,ancestors')):
     child_id=str(child['id']);remote_position=child.get('position')
     if child_id in pages:positions[child_id]=remote_position if isinstance(remote_position,int) and remote_position>=0 else position
  converted_pages={}
  for cid,p in pages.items():
   if str(p['space']['id'])!=space_id:raise RuntimeError('Selected page belongs to another space')
   parent_id=str(p['ancestors'][-1]['id']) if p.get('ancestors') else None
   converted_pages[cid]={
    'id':cid,
    'title':p['title'],
    'spaceId':str(p['space']['id']),
    'version':{'number':p['version']['number']},
    'parentId':parent_id,
    'position':positions.get(cid,p.get('position'))
   }
  pages=converted_pages
  for cid,p in pages.items():
   nodes[cid]={'id':cid,'type':'page','title':p['title'],'parentId':p['parentId'],'position':positions.get(cid,p.get('position'))}
...
def page_paths(nodes,pages,old):
 ...
 # Rebuild source paths from the current remote hierarchy. Page IDs keep identity
 # stable while a later explicit working-file replacement adopts moves/renames.
 paths={}
 dirs={};visiting=set()
...
def main():
 global ORIGIN, API_MODE
 c=load_config();ORIGIN=c['siteUrl'];API_MODE=c.get('apiMode','cloud');scope_id=digest(json.dumps(c,sort_keys=True).encode())
 original=MP.read_bytes() if MP.exists() else None;prior=json.loads(original) if original else {'pages':[]}
 legacy_config={k:v for k,v in c.items() if k!='apiMode'}
 legacy_scope_id=digest(json.dumps(legacy_config,sort_keys=True).encode()) if API_MODE=='cloud' else None
 changed_scope=prior.get('scope_id')!=scope_id and not (legacy_scope_id and prior.get('scope_id')==legacy_scope_id)
 ...
 moved_links={item['path'].removesuffix('.md'):paths[cid].removesuffix('.md') for cid,item in old.items() if cid in paths and item['path']!=paths[cid]}
 def relocate_links(content):
  # Rewrite only exact vault-path wiki targets generated by this importer.
  # One substitution pass also handles swaps without cascading replacements.
  return re.sub(r'\[\[([^\]#|]+)([^\]]*)\]\]',lambda match:'[['+moved_links.get(match[1],match[1])+match[2]+']]',content.decode()).encode()
 now=datetime.now(timezone.utc).isoformat(timespec='seconds');writes={};items=[];updated=0
 for cid,p in pages.items():
  prev=old.get(cid);version=p['version']['number']
  path=confined(paths[cid])
  if path.exists() and path not in originals:raise RuntimeError('New import target already exists')
  if prev and prev['version']==version and prior.get('content_format')==CONTENT_FORMAT:
   item=dict(prev);item.pop('not_seen_at',None);item['title']=p['title'];item['remote_title']=p['title'];item['parent_id']=nodes[cid]['parentId'];item['source_order']=source_order(nodes,cid);item['path']=paths[cid]
   original_content=originals[confined(prev['path'])]
   content=relocate_links(original_content)
   if prev['path']!=paths[cid] or content!=original_content:
    writes[path]=content;item['sha256']=digest(content);updated+=1
   items.append(item);continue
  if API_MODE=='server':
   detail=req('/content/'+cid+'?expand=body.storage,version')
   if detail['version']['number']!=version:raise RuntimeError('Source changed during refresh')
   raw=detail['body']['storage']['value']
  else:  # cloud mode
  ...
  for a in soup.find_all('a',href=True):
   href=urllib.parse.urljoin(ORIGIN.rstrip('/')+'/',a['href']);parsed=urllib.parse.urlsplit(href)
   if parsed.netloc!=urllib.parse.urlsplit(ORIGIN).netloc:continue
   match=re.search(r'/pages/(\d+)',parsed.path)
   target=match[1] if match else urllib.parse.parse_qs(parsed.query).get('pageId',[None])[0]
  ...
  if API_MODE=='server':
   url=ORIGIN+'/pages/viewpage.action?pageId='+cid
  else:  # cloud mode
 ...
 warning_pages=sum(bool(item.get('warnings')) for item in items if not item.get('not_seen_at'))
 ...
 if API_MODE=='server':
  site_type='Confluence Data Center'
 else:  # cloud mode
  site_type='Confluence Cloud'
 writes[ROOT/'6 Import log/Confluence/Latest refresh.md']=(f'# Latest source refresh\n\nChecked: {now}\n\n- Site: {ORIGIN} ({site_type})\n- Space: {c["spaceKey"]}\n- Selection: {c["scope"]["mode"]}\n- {len(pages)} pages available\n- {updated} pages added or updated\n- {warning_pages} pages with conversion warnings\n- {len(missing)} missing pages retained\n- {len(skipped)} unsupported content items skipped\n\nWorking files were not replaced. Configuration: [[7 Tools/Confluence source]].\n').encode()
 if '--dry-run' in sys.argv:print(json.dumps({'checked':len(pages),'updated':updated,'warning_pages':warning_pages,'scope_changed':changed_scope,'titles':[p['title'] for p in pages.values()],'unsupported':skipped}));return
 ...
 referenced={confined(item['path']) for item in items}
 removals=[p for p in originals if p not in referenced]
 backups={};done=[]
 try:
  ...
  raise
 print(json.dumps({'checked':len(pages),'updated':updated,'warning_pages':warning_pages,'missing_retained':missing,'scope_changed':changed_scope,'archive':str(archive.relative_to(ROOT)) if archive else None}))
